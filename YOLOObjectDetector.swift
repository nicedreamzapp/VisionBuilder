...
class YOLOObjectDetector {
    ...
    /// YOLOE (4585 classes, open-vocabulary prompt-free) has decode baked into
    /// the CoreML graph — outputs confidence/class_id/boxes per anchor.
    enum Generation { case v8, v26, yoloE }
    ...
    private let yoloEConfidenceThreshold: Float = 0.35  // 4585 classes need a higher bar
    private let iouThreshold: Float = 0.45
    private let maxDetections = 20  // was 5 — handle photos with many objects
    private let inputSize: Int = 640

    /// Preferred order: YOLOE if bundled, else YOLO26, else YOLOv8 OIV7.
    private enum ModelNames {
        static let yoloE = "yoloe11s_pf"
    ...
    func ensureLoaded() async throws {
        ...
        // Load the class names matching the loaded generation.
        // YOLOE = 4585 names (JSON array); YOLOv8 OIV7 = 601; YOLO26n = 80 COCO.
        if generation == .yoloE,
           let url = Bundle.main.url(forResource: "yoloe_classes", withExtension: "json"),
    ...
    /// YOLOE if bundled, else YOLO26, else YOLOv8 OIV7.
    private func resolvePreferredModelName() -> (name: String, gen: Generation) {
        if Bundle.main.url(forResource: ModelNames.yoloE, withExtension: "mlmodelc") != nil {
            return (ModelNames.yoloE, .yoloE)
        }
    ...
    func detect(in image: UIImage) async throws -> [DetectedObject] {
        ...
        // YOLOE: decode is in-graph; read the three ready-made outputs
        if loadedGeneration == .yoloE {
            return decodeYOLOEOutput(output, imageWidth: imageWidth, imageHeight: imageHeight, letterbox: letterbox)
        }
    ...
    // MARK: - Decode YOLOE (per-anchor max already computed in-graph)
    ...
    private func decodeYOLOEOutput(
        ...
        guard let confArr = output.featureValue(for: "confidence")?.multiArrayValue,
              let clsArr = output.featureValue(for: "class_id")?.multiArrayValue,
              let boxArr = output.featureValue(for: "boxes")?.multiArrayValue else {
        ...
        for i in 0..<numAnchors {
            ...
            // boxes layout [1, 4, anchors] xywh in 640 space
            let cx = floatAt(boxArr, i)
            let cy = floatAt(boxArr, numAnchors + i)
            let w = floatAt(boxArr, 2 * numAnchors + i)
            let h = floatAt(boxArr, 3 * numAnchors + i)
            ...
            let rect = CGRect(
                x: CGFloat(max(0, (origCx - origW / 2)) / Float(imageWidth)),
                y: CGFloat(max(0, (origCy - origH / 2)) / Float(imageHeight)),
                width: CGFloat(min(origW / Float(imageWidth), 1.0)),
                height: CGFloat(min(origH / Float(imageHeight), 1.0))
            )
